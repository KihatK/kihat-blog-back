...
const { Bcategory, Scategory, Post } = require('../models');
...
router.delete('/:bcategory', isAdminLoggedIn, async (req, res, next) => {
    try {
        ...
        await Promise.all(scategories.map(async s => {
            const posts = await Post.findAll({
                where: { ScategoryId: s.id },
                order: [['createdAt', 'DESC']],
            });
            return await Promise.all(posts.map(p => {
                return Post.destroy({ where: { id: p.id } })
            }));
        }));
        await Promise.all(scategories.map(s => {
